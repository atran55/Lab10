...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    unsigned int delta_time = stop_time-start_time;
    // read 4 bytes 2048 times, so in total 16384 bytes read over
    unsigned int bytes_transferred = 16384; // change obviously
    float time_spent = delta_time/125.0e6;; // change obviously

    throughput = bytes_transferred/time_spent; 
    throughput = throughput/1048576.0; // convert from bytes to Mbytes (do it after so the compiler doesn't do anything weird)
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 10 Andrew Tran - Custom Peripheral Demonstration\n\r");
    *(my_periph+RADIO_TUNER_CONTROL_REG_OFFSET) = 0; // custom-added; do initial reset
